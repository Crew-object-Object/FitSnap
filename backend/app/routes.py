...
async def predict_size(payload: dict):
    ...
    if not user_url:
        return {"error": "user_url is required"}
    
    # Download image from user_url
    response = requests.get(user_url)
    if response.status_code != 200:
        return {"error": "Failed to retrieve image from user_url"}
    image = cv2.imdecode(np.frombuffer(response.content, np.uint8), cv2.IMREAD_COLOR)
    if image is None:
        return {"error": "Invalid image retrieved from user_url"}
    ...
    # Simple size mapping based on chest width
    if chest_width < 80:
        shirt_size = "XXS"
    elif chest_width < 90:
        shirt_size = "XS"
    ...
    # Download image from fit_url if provided
    if fit_url:
        response = requests.get(fit_url)
        if response.status_code != 200:
            return {"error": "Failed to retrieve image from fit_url"}
    ...
    if (fit_results.pose_landmarks and fit_results.pose_landmarks[0] and 
        ...
        # Compute angles for shoulders in degrees
